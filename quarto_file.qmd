...
To examine NDVI patterns in more detail, I visualized temporal trends of NDVI over the sampling period @fig-ndvi_time. These smoothed trends highlight how vegetation greenness changes over time and reveal differences between site types. Next, I explored seasonal variation by aggregating NDVI values by month @fig-ndvi_seasonal, which shows typical seasonal patterns such as lower NDVI in winter and higher NDVI in summer.

To assess the overall distribution of NDVI across sites, I created site-level histograms @fig-ndvi_distribution, revealing variation in NDVI between locations. The density plot @fig-ndvi_density shows the distribution of NDVI values across all observations and highlighting differences between site types. Finally, I calculated NDVI anomalies at the plot level @fig-ndvi_anomaly to identify detect unusual patterns at site level. For the context of wind energy I would expect decrease of NDVI during and after construction, fro example.

```{r}
#| echo: false
#| label: fig-ndvi_time
#| fig-cap: "NDVI trends over time (smoothed)"
ndvi_time_plot
```
...
#| fig-cap: "Seasonal patterns"
ndvi_seasonal_plot    
```

```{r}
#| echo: false
#| label: fig-ndvi_distribution
#| fig-cap: "Distribution by site"
...
```{r}
#| echo: false
#| label: fig-ndvi_anomaly
#| fig-cap: "Anomalies on plot level"
ndvi_anomaly_plot 
```
